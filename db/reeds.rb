...
location_tag_hash = {}
CSV.foreach("#{Rails.root}/app/assets/data/health_facilities_bt.csv", headers: true, encoding: 'ISO-8859-1') do |row|
  next if row[5].blank?
...
couchdb_user = User.where(username: 'admin')
sleep 3



CSV.foreach("#{Rails.root}/app/assets/data/health_facilities_additional_sites.csv", headers: true, encoding: 'ISO-8859-1') do |row|
  next if row[3].blank?
  ...
  mga             = row[6].squish
  geographic_area = row[7].squish rescue nil
  latitude        = row[8].squish rescue nil
  longitude       = row[9].squish rescue nil

  ActiveRecord::Base.transaction do
    district  = Location.where(name: district_name).first

    couchdb_facility = CouchdbLocation.create(name: name, code: facility_code, 
      creator: couchdb_user.first.couchdb_user_id, longitude: longitude, latitude: latitude)

    facility = Location.create(name: name, code: facility_code, 
      creator: couchdb_user.first.user_id, longitude: longitude, latitude: latitude,couchdb_location_id: couchdb_facility.id)
=begin
    [zone, facility_type, mga, geographic_area].each do |lt|
      ...
      LocationTagMap.create(location_id: facility.id, 
			  location_tag_id: lt_location_tag.id,p
