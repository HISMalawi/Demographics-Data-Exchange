...
class SyncJob < ApplicationJob
  ...
  def push_footprints
    ...
    footprints = if @is_mahis_sync
                   FootPrint.where(synced: false)
                 else
                   FootPrint.where(synced: false, location_id: @location)
                 end

    footprints.find_in_batches(batch_size: 50) do |batch|
