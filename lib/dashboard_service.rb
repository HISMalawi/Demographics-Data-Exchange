module DashboardService
  ...
  def self.new_reg_by_site
    PersonDetail.joins("JOIN locations l ON person_details.location_created_at = l.location_id").where("date(date_registered) = date(last_edited)").group(:name).count
  end
  ...
  def self.new_reg_past_30
    result = PersonDetail.find_by_sql('SELECT l.name,date(date_registered),count(*) registrations from person_details pd
      ...
      WHERE date(date_registered) = date(last_edited)
      AND date(date_registered) >= DATE_SUB(date(now()), INTERVAL 30 DAY)
