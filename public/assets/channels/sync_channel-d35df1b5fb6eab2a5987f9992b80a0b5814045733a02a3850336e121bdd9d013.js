...
function updateSyncStatus(data) {
  ...
  statusElement.textContent = `[${timestamp}] ${data.message}`
  ...
  if (data.level === "success") {
    ...
    setTimeout(() => {
      statusElement.textContent = `[${timestamp}] Synchronization complete ✅`
    }, 1000)
  }
  ...
  if (data.level === "failed" || data.level === "error") {
    ...
    setTimeout(() => {
      statusElement.textContent = `[${timestamp}] Synchronization failed ❌`
    }, 1000)
  }
}
;
